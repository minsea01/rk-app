...
class TestExportYOLOv8ToONNX:
    ...
    def test_calls_model_export_with_correct_parameters(self, temp_dir, mock_ultralytics):
        ...
        mock_model.export.assert_called_once_with(
            format="onnx", imgsz=416, opset=11, simplify=False, dynamic=True, half=True
        )
...
class TestMainFunction:
    ...
    def test_main_returns_zero_on_success(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch(
                "tools.export_yolov8_to_onnx.export", return_value=Path(tmpdir) / "model.onnx"
            ):
                mock_args = ["--weights", str(weights_path), "--imgsz", "640", "--outdir", tmpdir]
    ...
    def test_main_returns_one_on_model_load_error(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch(
                "tools.export_yolov8_to_onnx.export", side_effect=ModelLoadError("Load failed")
            ):
                mock_args = ["--weights", str(weights_path), "--imgsz", "640", "--outdir", tmpdir]
                with patch("sys.argv", ["export_yolov8_to_onnx.py"] + mock_args):
                    result = main()
    ...
    def test_main_returns_one_on_configuration_error(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch(
                "tools.export_yolov8_to_onnx.export", side_effect=ConfigurationError("Config error")
            ):
                mock_args = ["--weights", str(weights_path), "--imgsz", "640", "--outdir", tmpdir]
                with patch("sys.argv", ["export_yolov8_to_onnx.py"] + mock_args):
                    result = main()
    ...
    def test_main_returns_one_on_unexpected_error(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch(
                "tools.export_yolov8_to_onnx.export", side_effect=Exception("Unexpected error")
            ):
                mock_args = ["--weights", str(weights_path), "--imgsz", "640", "--outdir", tmpdir]
                with patch("sys.argv", ["export_yolov8_to_onnx.py"] + mock_args):
                    result = main()
