...
测试集会持续演进，具体数量请以当前仓库中的 `pytest` / `ctest` 输出为准。
...
- 优先覆盖异常路径、回退逻辑、线程/队列边界和配置解析。
- 避免在文档中固化测试数量、覆盖率等容易过期的统计值。
- 任何流程变更都应同步更新本指南中的命令与前置条件。
...
### 当前状态
- 以当前 `pytest tests/unit -m "not requires_hardware"` 与 `ctest --preset x86-debug` 的实际结果为准。
- 覆盖率请通过 `pytest --cov=apps --cov=tools --cov-report=term` 现算，不在文档中维护硬编码数字。
