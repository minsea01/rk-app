...
if "MV-CA020-20GC" not in str(source.get("uri", "")):
    errors.append("source.uri should target MV-CA020-20GC by default")
model = root / str(engine.get("model", ""))
if not model.exists():
    errors.append(f"model missing: {model}")
if output.get("host") != "192.168.137.1":
    errors.append("output.tcp.host should be 192.168.137.1")
