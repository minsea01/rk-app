...
def convert_deeppcb_annotation(txt_file, target_dir, class_mapping, image_width=640, image_height=640):
...
def main():
    ...
    deeppcb_classes = ['open', 'short', 'mousebite', 'spur', 'copper', 'pinhole']
    all_class_names = sorted(list(set(neu_classes + deeppcb_classes))) # Use sorted set for consistency
    ...
    for xml_file in tqdm(glob(os.path.join(neu_annots_path, '*.xml'))):
        ...
        if os.path.exists(os.path.join(LBL_DIR, label_filename)):
            all_files_info.append({
                "img_src": os.path.join(neu_images_path, img_filename),
                "lbl_name": label_filename
            })
...
"""
    ...
    print(f"YAML config file is at: {os.path.abspath(os.path.join(FINAL_DATASET_DIR, 'data.yaml'))}")
