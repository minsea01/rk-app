...
def main():
    ...
    ap.add_argument(
        "--labels-root", default=None, help="labels root (default: infer from images path)"
    )
    ap.add_argument("--img-root", default=None, help="images root (default: take from JSON)")
    ap.add_argument(
        "--iou",
        default="0.50:0.95:0.05",
        help="IoU thresholds as start:end:step or single, e.g., 0.5 or 0.50:0.95:0.05",
    )
    args = ap.parse_args()
    ...
    if args.img_root:
        ...
        if len(src_dirs) != 1:
            raise RuntimeError(
                f"Cannot infer a unique images folder from JSON (got {src_dirs}). Use --img-root"
            )
    ...
    for cls in range(nc):
        ...
        if aps_th:
            ...
            per_class_output.append(
                (cls, names[cls] if cls < len(names) else str(cls), ap50, ap_mean)
            )
            if ap50 is not None:
                ap50s.append(ap50)
