...
class StageStats:
    ...
    def summary(self) -> Dict[str, float]:
        ...
        return {
            "n": self.n,
            "avg_ms": avg * 1000,
            "min_ms": self.t_min * 1000,
            "max_ms": self.t_max * 1000,
        }
        ...
        while not stop.is_set():
            ...
            if not ok:
                ...
                if reconnect_attempts % 10 == 1:
                    logger.warning(
                        "Capture read failed; reconnecting (attempt %d)", reconnect_attempts
                    )
                cap.release()
        ...
        while not stop.is_set():
            ...
            if writer:
                ...
                writer = cv2.VideoWriter(
                    str(args.save), fourcc, max(1, args.fps or 25), writer_size
                )
                if writer.isOpened():
                    writer.write(frame)
                else:
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        type=str,
        default="0",
        help="camera index (as string) or gstreamer/rtsp/file path",
    )
    ...
    p.add_argument(
        ...
        action="store_true",
        default=False,
        help="show result window (auto-fallback to save when headless)",
    )
    p.add_argument("--save", type=Path, default=None)
    p.add_argument("--max-frames", type=int, default=0)
    p.add_argument("--upload-http", type=str, default=None, help="POST detections JSON to this URL")
    p.add_argument(
        "--upload-interval", type=float, default=0.0, help="seconds between uploads (throttle)"
    )
    p.add_argument(
        "--log-level",
        type=str,
